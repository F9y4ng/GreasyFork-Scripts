...
// @note            修正Gecko内核因transform缩放造成的位移。\n优化CSS样式，解决一些特殊情况下的样式bug。\n修正bugs, 优化代码。
// @grant           GM_info
...
!(function () {
  ...
  const isGecko = (u = navigator.userAgent.toLowerCase()) => {
    return u.includes("gecko") && !u.includes("khtml");
  };
  ...
  GMnotification = (text = "", type = `${Notice.info}`, closeWith = true, timeout = 30, { ...options } = {}, position = "bottomRight") => {
    try {
      ...
      transformFixedtoScroll(position);
    } catch (e) {
