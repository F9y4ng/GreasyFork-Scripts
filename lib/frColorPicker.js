...
var $frColorPicker = $frColorPicker || {};
$frColorPicker.scope = {};
$frColorPicker.arrayIteratorImpl = function (q) {
  var C = 0;
  return function () {
    return C < q.length ? { done: !1, value: q[C++] } : { done: !0 };
  };
};
$frColorPicker.arrayIterator = function (q) {
  return { next: $frColorPicker.arrayIteratorImpl(q) };
};
$frColorPicker.makeIterator = function (q) {
  var C = "undefined" != typeof Symbol && Symbol.iterator && q[Symbol.iterator];
  return C ? C.call(q) : $frColorPicker.arrayIterator(q);
};
(function (q, C) {
  "object" === typeof module && "object" === typeof module.exports
    ? (module.exports = q.document
        ? C(q)
        : function (D) {
            if (!D.document) throw Error("No window with document!");
            return C(D);
          })
    : C(q);
})("undefined" !== typeof window ? window : this, function (q, C) {
  function D(a) {
    var b = "",
      d = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".concat("0123456789", "abcdefghijklmnopqrstuvwxyz");
    for (a = Number(a - 1) || 8; 0 < a; --a) b += d[Math.floor(Math.random() * d.length)];
  ...
  var I = D(9),
    ...
    R = D(4);
  C =
    q.self === q.top && q.trustedTypes && q.trustedTypes.createPolicy
      ? q.trustedTypes.createPolicy(
          (function (a) {
            a = void 0 === a ? "fr#ColorPicker" : a;
            var b = new Set([
              { host: "teams.live.com", policy: "gapi#gapi" },
              { host: "github.dev", policy: "standaloneColorizer" },
            ]);
            b = $frColorPicker.makeIterator(b.values());
            for (var d = b.next(); !d.done; d = b.next())
              if (((d = d.value), location.hostname.startsWith(d.host))) {
                a = d.policy;
                break;
              }
            return a;
  ...
  var J = (function () {
    var a = {
      ...
      node: function (b) {
        ...
        if ("string" === typeof b) {
          ...
          d || console.warn("No element matches the selector: %s", b);
          return d;
        }
      ...
      genColorPreviewCanvas: function (b, d, e, g) {
        ...
        b && ((z.fillStyle = b), z.fillRect(0, 0, e, x));
        b = null;
        switch (d) {
          case "left":
            b = 0;
            z.clearRect(0, 0, h / 2, x);
            break;
          case "right":
            (b = e - h), z.clearRect(e - h / 2, 0, h / 2, x);
        }
        if (null !== b)
          for (z.lineWidth = 1, d = 0; d < a.pub.previewSeparator.length; d += 1)
            z.beginPath(), (z.strokeStyle = a.pub.previewSeparator[d]), z.moveTo(0.5 + b + d, 0), z.lineTo(0.5 + b + d, x), z.stroke();
        return { canvas: u, width: e, height: x };
      },
        ...
        if (a.picker && a.picker.owner) {
          ...
          e = b.smartPosition
            ? [
                ...
                -e[r] + d[r] + g[r] + l[r] - u + u * x > h[r]
                  ? -e[r] + d[r] + g[r] / 2 > h[r] / 2 && 0 <= d[r] + g[r] - u - u * x
                    ? d[r] + g[r] - u - u * x - 4
                    : d[r] + g[r] - u + u * x + 4
                  : 0 <= d[r] + g[r] - u + u * x
                  ? d[r] + g[r] - u + u * x - 4
      ...
      _drawPosition: function (b, d, e, g, h) {
        ...
        var l = "CSS1Compat" === document.compatMode ? document.documentElement : document.body,
          m = q.getComputedStyle(document.body, null).getPropertyValue("transform");
      ...
      getPaletteDims: function (b, d) {
        ...
        return { cols: e, rows: g, cellW: h, cellH: l, width: d, height: m };
      },
      ...
      onControlPointerStart: function (b, d, e, g) {
        ...
        a.preventDefault(b);
        var l = function (r, x) {
          a.attachGroupEvent("drag", r, a._pointerMoveEvent[g], a.onDocumentPointerMove(b, d, e, g, x));
          a.attachGroupEvent("drag", r, a._pointerEndEvent[g], a.onDocumentPointerEnd(b, d, e, g));
        };
        l(q.document, [0, 0]);
        if (q.parent && q.frameElement) {
          var m = q.frameElement.getBoundingClientRect();
          l(q.parent.window.document, [-m.left, -m.top]);
        }
      ...
      pub: function (b, d) {
        ...
        function g() {
          ...
          var c = a.picker,
            ...
            y = a.getControlPadding(f),
            A = Math.min(f.borderRadius, Math.round(f.padding * Math.PI)),
            v = q.getComputedStyle(document.body, null).getPropertyValue("transform");
          v = v && "none" !== v ? Number(v.split(",")[3]) : 0;
          var w = Number(q.getComputedStyle(document.body, null).getPropertyValue("zoom")) || 1;
          c.wrap.className = L;
          ...
          v && 1 !== v
            ? ((c.wrap.style.transformOrigin = "left top 0px"), (c.wrap.style.transform = "scale(" + 1 / v + ")"))
          ...
          c.padM.style.width = f.padding + 2 * f.controlBorderWidth + f.width + y + "px";
          ...
          c.sldB.style.left = f.padding + f.width + 2 * f.controlBorderWidth + 2 * y + "px";
          ...
          c.sldM.style.left = f.padding + f.width + 2 * f.controlBorderWidth + y + "px";
          c.sldM.style.top = "0px";
          c.sldM.style.width = f.sliderSize + 2 * y + 2 * f.controlBorderWidth + (p ? 0 : Math.max(0, f.padding - y)) + "px";
          ...
          c.asldB.style.left = f.padding + f.width + 2 * f.controlBorderWidth + y + (k ? f.sliderSize + 3 * y + 2 * f.controlBorderWidth : 0) + "px";
          ...
          c.asldM.style.left = f.padding + f.width + 2 * f.controlBorderWidth + y + (k ? f.sliderSize + 2 * y + 2 * f.controlBorderWidth : 0) + "px";
          c.asldM.style.top = "0px";
          c.asldM.style.width = f.sliderSize + 2 * y + 2 * f.controlBorderWidth + Math.max(0, f.padding - y) + "px";
          ...
          for (t = p = 0; t < n.palette.rows; t++)
            for (y = 0; y < n.palette.cols && p < f._palette.length; y++, p++) {
              A = f._palette[p];
              w = a.rgbaColor.apply(null, A.rgba);
              v = a.createEl("div");
              v.style.width = n.palette.cellW - 2 * f.controlBorderWidth + "px";
              v.style.height = n.palette.cellH - 2 * f.controlBorderWidth + "px";
              v.style.backgroundColor = w;
              w = a.createEl("div");
              w.className = Q;
              w.style.display = "block";
              w.style.position = "absolute";
              w.style.left = (1 >= n.palette.cols ? 0 : Math.round(((n.contentW - n.palette.cellW) / (n.palette.cols - 1)) * 10 * y) / 10) + "px";
              w.style.top = t * (n.palette.cellH + f.paletteSpacing) + "px";
              w.style.border = f.controlBorderWidth + "px solid";
              w.style.borderColor = f.controlBorderColor;
              w.style.cursor = "pointer";
              if (null !== A.rgba[3] && 1 > A.rgba[3]) {
                try {
                  w.style.backgroundImage = "url('" + k.canvas.toDataURL() + "')";
                } catch (E) {
                  w.style.backgroundImage =
                    "url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACMAAAAQCAYAAACcN8ZaAAAAAXNSR0IArs4c6QAAAFNJREFUSEtjnDlz5n8GPODs2bP4pBmMjY3xypOin3HUMUhhiRyyoyGDnMhGQwZXlhu8IZOWloa3nKFmOYItdJDLIcZRxyAF0WjI4MpNoyEz5EIGAMmNh+nDrPy/AAAAAElFTkSuQmCC')";
                }
              ...
              w.addEventListener("click", a.onPaletteSampleClick, !1);
              w.appendChild(v);
              c.pal.appendChild(w);
            }
          c.btn.className = R;
          ...
          (function () {
            var E = f.controlBorderColor.split(/\s+/);
            c.btn.style.borderColor = 2 > E.length ? E[0] : E[1] + " " + E[0] + " " + E[0] + " " + E[1];
          })();
        ...
        this.randomize = function (c, k, p, n, t, y, A, v) {
          void 0 === c && (c = 0);
          void 0 === k && (k = 100);
          void 0 === p && (p = 0);
          void 0 === n && (n = 100);
          ...
          void 0 === v && (v = 1);
          this.fromHSVA(
            t + Math.floor(Math.random() * (y - t + 1)),
            p + Math.floor(Math.random() * (n - p + 1)),
            c + Math.floor(Math.random() * (k - c + 1)),
            (100 * A + Math.floor(Math.random() * (100 * (v - A) + 1))) / 100
          );
        };
        ...
        this.toDataURL = function () {
          try {
            ...
            return "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACMAAAAQCAYAAACcN8ZaAAAAAXNSR0IArs4c6QAAAFNJREFUSEtjnDlz5n8GPODs2bP4pBmMjY3xypOin3HUMUhhiRyyoyGDnMhGQwZXlhu8IZOWloa3nKFmOYItdJDLIcZRxyAF0WjI4MpNoyEz5EIGAMmNh+nDrPy/AAAAAElFTkSuQmCC";
          }
        };
        ...
        this.setPreviewElementBg = function (c) {
          if (this.previewElement) {
            ...
            c = [];
            p = [];
            for (var t = [], y = [], A = [], v = 0; v < n.length; v += 1) c.push(n[v].image), p.push(n[v].position), t.push(n[v].size), y.push(n[v].repeat), A.push(n[v].origin);
            n = {
              "background-image": c.join(", "),
              "background-position": p.join(", "),
              "background-size": t.join(", "),
              "background-repeat": y.join(", "),
              "background-origin": A.join(", "),
            };
            a.setStyle(this.previewElement, n, this.forceStyle);
            n = { left: null, right: null };
            k && (n[k] = this.previewSize + this.previewPadding + "px");
            n = { "padding-left": n.left, "padding-right": n.right };
            a.setStyle(this.previewElement, n, this.forceStyle, !0);
          }
        };
        this.setValueElementValue = function (c) {
          this.valueElement && ("input" === a.nodeName(this.valueElement) ? (this.valueElement.value = c) : (this.valueElement.innerHTML = C.createHTML(c)));
        };
        this.setAlphaElementValue = function (c) {
          this.alphaElement && ("input" === a.nodeName(this.alphaElement) ? (this.alphaElement.value = c) : (this.alphaElement.innerHTML = C.createHTML(c)));
        };
        ...
        if (a.pub.options)
          for (var B in a.pub.options)
            if (a.pub.options.hasOwnProperty(B))
              try {
                e(B, a.pub.options[B]);
              } catch (c) {
                console.error(c);
        ...
        var F = [];
        d.preset && ("string" === typeof d.preset ? (F = d.preset.split(/\s+/)) : Array.isArray(d.preset) ? (F = d.preset.slice()) : console.warn("Unrecognized preset value"));
        -1 === F.indexOf("default") && F.push("default");
        for (var H = F.length - 1; 0 <= H; --H) {
          var G = F[H];
          if (G && a.pub.presets.hasOwnProperty(G))
            for (B in a.pub.presets[G])
              if (a.pub.presets[G].hasOwnProperty(B))
                try {
                  e(B, a.pub.presets[G][B]);
                } catch (c) {
                  console.error(c);
                }
        }
        F = ["preset"];
        for (B in d)
          if (d.hasOwnProperty(B) && -1 === F.indexOf(B))
            try {
              e(B, d[B]);
            } catch (c) {
              console.error(c);
        ...
        this.valueElement &&
          ...
          this.valueElement.addEventListener("keydown", x, !1),
          this.valueElement.addEventListener("change", z, !1),
          this.valueElement.addEventListener("input", T, !1),
        ...
        this.alphaElement &&
          ...
          this.alphaElement.addEventListener("change", S, !1),
          this.alphaElement.addEventListener("input", U, !1),
        ...
        -1 < ["auto", "any"].indexOf(this.format.toLowerCase())
          ? (this._currentFormat = (B = a.parseColorString(b)) ? B.format : "hex")
    ...
    a.pub.background = function (b) {
      ...
      try {
        ...
        d.push(
          "url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACMAAAAQCAYAAACcN8ZaAAAAAXNSR0IArs4c6QAAAFNJREFUSEtjnDlz5n8GPODs2bP4pBmMjY3xypOin3HUMUhhiRyyoyGDnMhGQwZXlhu8IZOWloa3nKFmOYItdJDLIcZRxyAF0WjI4MpNoyEz5EIGAMmNh+nDrPy/AAAAAElFTkSuQmCC') left top repeat"
        );
      }
      return d.join(", ");
    };
  ...
  "undefined" === typeof q.FRColorPicker && (q.FRColorPicker = q.frColorPicker = J);
  return J;
});
