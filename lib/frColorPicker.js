  ...
  var T = (function () {
    var a = {
      ...
      hexColor: function (b, d, e) {
        return "#" + (("0" + Math.round(b).toString(16)).substr(-2) + ("0" + Math.round(d).toString(16)).substr(-2) + ("0" + Math.round(e).toString(16)).substr(-2)).toUpperCase();
      },
      ...
      pub: function (b, d) {
        ...
        for (var G = C.length - 1; 0 <= G; --G) {
          ...
          if (F && a.pub.presets.hasOwnProperty(F))
            for (A in a.pub.presets[F])
              if (a.pub.presets[F].hasOwnProperty(A))
                try {
                  e(A, a.pub.presets[F][A]);
                } catch (c) {}
        }
        C = ["preset"];
        for (A in d)
