  ...
  !(async function () {
      ...
      if (isMutationObserver) {
        ...
        document.onreadystatechange = function () {
          if (document.readyState === "complete") {
            const isStyleInLastChild = document.head.lastChild.className === defCon.class.rndStyle;
            debug("//-> isStyleInLastChild:", isStyleInLastChild);
            if (!isStyleInLastChild) {
              insertStyle(true);
            }
          }
        };
        ...
        backupT.addEventListener("click", async () => {
          try {
            ...
            if (await frDialog.respond()) {
              ...
              const db_R = defCon.encrypt(`\u26a1${GMinfo.scriptMetaStr.match(/(\u0074\u0068\u006f\u0072\s+)(\S+)/)[2]}\ud83d\udc96${defCon.scriptName}`);
              ...
              let frDialog = new frDialogBox({
                trueButtonText: "确 定",
                messageText: `<p>备份数据已归档，备份文件导出下载中……</p><p>文件名：<span style="color:darkred;font-size:12px">backup.${timeStamp}${via}.sqlitedb</span></p>`,
                titleText: "数据备份",
              });
              if (await frDialog.respond()) {
                frDialog = null;
    ...
    async function manageDomainList(_temp_ = [], Contents = "") {
      ...
      let frDialog = new frDialogBox({
        ...
        neutralButtonText: "取 消",
        messageText: `<p style="font-size:14px;color:darkred">请谨慎操作，保存后生效，已删除的数据将不可恢复！</p>${_data_search_}<ul id="${defCon.id.seed}_d_d_" style="margin:0;padding:0;list-style:none;overflow:auto;max-height:190px;white-space:nowrap">${Contents}</ul>`,
