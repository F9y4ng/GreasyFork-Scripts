...
(function () {
  ...
  let shadow_r = 4; // 字体阴影：建议控制在1~8之间，关闭阴影为0，默认4
  const smooth_i = 1; // 字体平滑，默认开启，关闭为0
  const shadow_c = `#888`; // 阴影颜色：建议#888，或 rgba(136,136,136,0.8) 或依喜好修改，currentcolor为原字体颜色（慎用）
  ...
  if (!isNaN(shadow_r) && shadow_r !== 0) {
    shadow = `text-shadow: 0 0 ${shadow_r}px ${shadow_c}!important;`;
  }
  let stroke = '';
