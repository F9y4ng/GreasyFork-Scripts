...
!(function () {
  ...
  !(async function () {
    ...
    const prefont = CONST.fontSelect.toString().split(",")[0];
    ...
    if (fontface_i) {
      fontfamily = `font-family:${CONST.fontSelect.toString()};`;
      fontface =
        refont !== ""
        ...
        if (t && d) {
          if (t.type !== "text") {
            ...
            t.addEventListener(method, () => {
              ...
              setEffectIntoSubmit(value, e, v, t, ii, d);
            });
          } else {
            Object.defineProperty(t, "_value_", {
              ...
              set: function (newVal) {
                setEffectIntoSubmit(newVal, e, v, t, ii, d);
              },
            });
          }
        }
    ...
    function setEffectIntoSubmit(value, e, v, t, ii, d) {
      try {
    ...
    async function manageDomainList(_temp_ = [], Contents = "") {
      ...
      for (let i = 0; i < _domains_.length; i++) {
        Contents += `<li id="_d_d_l_${i}" style="list-style:none;font-size:14px;font-style:normal;padding:5px;color:#555">\
                [<span id="_d_d_l_s_${i}" style="padding:3px;cursor:pointer;color:crimson;font-size:14px">删除</span>]\
                <span>${i + 1}. ${_domains_[i].domain} - ${dateFormat("YYYY/mm/dd HH:MM:SS", new Date(_domains_[i].fontDate))}</span></li>`;
      }
      let frDialog = new frDialogBox({
