...
!(function () {
  ...
  !(async function () {
    ...
    const res = rebuild === undefined ? Val : rebuild;
    ...
    addLoadEvent(async () => {
      if (Number(sessionStorage.getItem("_temp_")) && Number(sessionStorage.getItem("_exSite_"))) {
        let frDialog = new frDialogBox({
          ...
          messageText: `<p><span style="font-size:28px;font-weight:900;color:red">您好</span>，这是您首次使用${defCon.scriptName}的新版本 <span style="font-family:Candara;color:darkorange;font-size:24px;font-weight:900;font-style:italic">V${defCon.curVersion}</span>，新版脚本解决了预载loading的样式错误、修正了部分页面字体显示错误的问题等等，具体功能敬请试用。</p><p>稍后，将为您打开新版脚本的新功能帮助文件页面，您需要去看一下吗？</p>`,
          titleText: "温馨提示",
        });
