...
void (function (ctx, FontRendering, proxyArrayMethods) {
  ...
  function (global, secures, protoMethods) {
    ...
    const def = {
      ...
      var: {
        curVersion: getMetaValue("version") ?? GMinfo.script.version ?? "2024.09.11.0",
    ...
    async function getNavigatorInfo() {
      ...
      async function getUserAgentDataFromExtension(credit) {
        ...
        const vmuad = GMscriptHandler === "Violentmonkey" && GMinfo.platform ? await getVMUserAgentData(GMinfo.platform) : null;
        const tmuad = GMscriptHandler === "!Tampermonkey" /* disabled for issues/2164 */ && GMinfo.userAgentData ? { ...GMinfo.userAgentData, credit } : null;
    ...
    void (async function (tTP, getRawJSONThroughBypass, getBrowserEnvironment) {
      ...
      const UPDATE_VERSION_NOTICE = IS_CHN
        ? `<li class="${def.const.seed}_add">新增对 Canvas 画布字体渲染的参数开关。</li>
            <li class="${def.const.seed}_fix">修复对 Canvas 画布字体排除渲染无效的问题。</li>
            <li class="${def.const.seed}_fix">修复渲染 Canvas 元素字体阴影偏移量未定义的问题。</li>
            <li class="${def.const.seed}_fix">修复一些已知的问题，优化代码，优化样式。</li>`
        : `<li class="${def.const.seed}_add">Added parameter switch for Canvas font rendering.</li>
            <li class="${def.const.seed}_fix">Fixed exclude not working for Canvas font rendering.</li>
            <li class="${def.const.seed}_fix">Fixed rendering Canvas font shadowOffset undefined.</li>
      ...
      void (async function (requestCodeAndFunc, getConfigureData, getCustomMonoData, getExSitesData, getFontSetData, getFontScaleDef, getFontOverrideDef) {
        ...
        const canvasLabel = IS_CHN
          ? "<label title='开启 Canvas 画布元素的字体渲染功能。如果开启后 Canvas 文本样式异常，请关闭之。'>渲染画布</label>"
          : "<label title='Enable Canvas font rendering function. Please turn it off if Canvas occurs style error.'>Canvas</label>";
        const tRenderCanvas = `<span id="${def.id.rdCanvas}">(${canvasLabel} <input type="checkbox" id="${def.id.renderCanvas}" ${CONST_VALUES.renderCanvas ? "checked" : ""}/>)</span>`;
        ...
        async function operateConfigure() {
          ...
          try {
            ...
            const canvasT = qS(`#${def.id.renderCanvas}`, def.const.configIf);
          ...
          function fontSelectionAndCustomFonts(fontSet) {
            try {
              if (fontSet) fontSet.fsearch();
              qS(`#${def.id.fonttooltip}`, def.const.configIf)?.addEventListener("dblclick", async e => {
                e.stopImmediatePropagation();
          ...
          function getFontShadow(shadowNode, shadowColorNode, submitButton) {
            ...
            const renderCanvasT = qS(`#${def.id.renderCanvas}`, def.const.configIf);
            try {
              ...
              saveChangeStatus(renderCanvasT, CONST_VALUES.renderCanvas, submitButton, def.array.values);
            }
          }
          ...
          function controlResetButton(resetT, fontSet, submitSet, { drawScale, drawStrock, drawShadow, submitButton }) {
            ...
            resetT?.addEventListener("click", async () => {
              ...
              if (await frDialog.respond()) {
                ...
                canvasT.checked !== INITIAL_VALUES.renderCanvas ? canvasT.click() : DEBUG("<canvas> NOT MODIFIED");
                ...
                changeNodeClass(shadowsT.value === "OFF", shadowColorNode, `${def.const.seed}_none`);
                canvasT.checked !== CONST_VALUES.renderCanvas ? canvasT.click() : DEBUG("<canvas> NOT MODIFIED");
        ...
        function overrideCanvasFont(renderFont) {
          if (!IS_CURRENTSITE_ALLOWED || !IS_INTERNALSTYLE_ALLOWED || !CONST_VALUES.renderCanvas) return;
          ...
          function handleAddedNodesMutation(addedNodes, mainStyle) {
            for (const node of addedNodes) {
              ...
              if (!mainStyle && insertMainStyleElement()) break;
                ...
                return Permission && deBounce({ fn: extraFunction, delay: 2e2, timer: `extra.${nodeName}` })(nodeName);
              }
            }
          }
        ...
        async (isFontsize, isFixViewport) => {
          ...
          const defaults = {
            ...
            renderCanvas: INITIAL_VALUES.renderCanvas,
            shadowColor: INITIAL_VALUES.shadowColor,
            fontCSS: INITIAL_VALUES.fontCSS,
            fontEx: INITIAL_VALUES.fontEx,
          ...
          try {
            ...
            return {
              ...
              renderCanvas: Boolean(currentValue.renderCanvas ?? true),
