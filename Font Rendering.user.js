  ...
  !(async function () {
    ...
    function insertCSS() {
      try {
        ...
        addStyle(tStyle, `${defCon.class.rndStyle}`, "head");
      } catch (e) {
    ...
    function startRAFInterval() {
      RAFInterval(
        () => {
          if (!qS(`.${defCon.class.rndClass}`) || !qS(`.${defCon.class.rndStyle}`)) {
            insertCSS();
          }
      ...
      return Boolean(qS(`.${defCon.class.rndClass}`) && qS(`.${defCon.class.rndStyle}`) && (!curWindowtop || qS(`#${defCon.id.rndId}`)));
    }
    ...
    async function manageDomainList(_temp_ = [], Contents = "") {
      ...
      if (qS(`#_d_s_`) && qS(`#_d_s_c_`) && qS(`#_d_s_s_`)) {
        qS(`#_d_s_`).addEventListener("keydown", e => {
