...
警告：**Tampermonkey 4.14**存在严重bug，请升级**Tampermonkey 4.15+**，或使用Violentmonkey替代！
...
```text
@ 精简内置字体表，优化字体表的全局兼容性。
@ 修正字体列表数据在字体反选时重复载入的问题。
@ Match @font-face { src: local(<name>) } names by PostScript and full font name for FontList.
@ 修正自定义字体添加保存数据的错误，数据去重验证。
@ 修正Blink内核异步载入iframe字体不渲染的Bug.(fix #68)
@ 修正一些细节问题，优化样式，优化代码。
```
...
> 每当计算机安装了新字体，要使其**立即生效**，请在“**高级核心功能设置**”中为当前网站重建字体列表缓存。
