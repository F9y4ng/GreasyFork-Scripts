...
```text
# 近期功能更新多、操作变更较大，建议阅读新版使用说明。
@ 优化字体列表缓存机制，由站点缓存变更为全局缓存。
@ 修正MacOS中在不同浏览器键盘快捷键冲突的问题。
@ 优化保存预览iframe框架及异步载入iframe的渲染机制。
@ 为“User-Agent Switcher and Manager”容错处理NavigatorUAData，但不保证脚本功能的完整性。(#70)
@ 修正其他兼容问题，修正配置页面样式错误，优化代码。
```
