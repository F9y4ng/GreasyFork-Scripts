...
- ## **Font Rendering.user.js** (`version 2021.07.23.4`)
  ...
  ```text
    ...
    + 发布保存预览功能（Beta.2）
    @ 优化预览按钮逻辑功能及新样式。
    @ 修正预览功能在同域iframe中失效的bug.
    @ 变更重置功能中的替换操作为恢复上一次正确保存的参数。
    @ 修正程序函数bugs，优化代码。

    # version 2021.07.22.2
    ! 本次更新会重置所有数据，请注意升级前备份数据！
    + 新增开源中文字体，霞鹜文楷、霞鹜新晰黑两款字体。
    + 新增高级功能设置页面，通过脚本菜单打开，不再需要修改代码使用高级功能。
    + 新增保存预览功能（实验性），默认关闭，需要在高级功能中开启。
    @ 优化脚本菜单，修正数据结构、修正CSS错误。
    @ 修正逻辑bugs，优化代码。
  ```
