...
```text
@ 修正包含拖拽方法网站的坐标位移问题（实验性）

+ 新增自定义字体添加辅助工具，优化输入格式的兼容性。
@ 优化脚本配置页面样式，修正CSSbugs，兼容更多网站。
@ 优化精简既定字体表，优化字体渲染效果。
@ 修正其他bugs，优化代码。
```

`以下被精简的字体表字体可选取自己所需添加至自定义字体表：`
```json
{"ch":"方正悠黑简体 508R","en":"FZYouHeiS 508R"}
{"ch":"字悦班马宋刻本","en":"ziyuebanmasongkeb"}
...
> >
> > **偏移问题：** Version 2021.11.07.1 已实验性修正因字体缩放引起的坐标偏移问题，如遇到坐标漂移、位移异常的网站，请反馈相关URL。
